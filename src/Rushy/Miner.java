...
public class Miner extends Robot {
    ...
    public void takeTurn() throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            for (int dx = -1; dx <=1; dx++) {
                for (int dy = -1; dy <=1; dy++) {
                    ...
                    while (rc.canMineLead(mineLocation) && rc.senseLead(mineLocation) > 1) {
                        ...
                        if(debugOn) rc.setIndicatorDot(rc.getLocation(), 0, 200, 0);
                        Debug.p("mined");
                        mined = true;
                    }
                }
            ...
            if(!mined){
                if(debugOn) rc.setIndicatorDot(rc.getLocation(),0,0,200);
                Debug.p("no mine");
            }

        }else{
            if(debugOn) rc.setIndicatorDot(rc.getLocation(),200,0,0);
            Debug.p("can't mine");
        }
    ...
    public void movement() throws GameActionException{
        ...
        if (nearby_enemy_units.length != 0) {
            if(debugOn) rc.setIndicatorString("moving away from enemy!");
