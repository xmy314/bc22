    ...
    */
    ...
    public static void init() throws GameActionException {
        ...
        if (rc.getType() == RobotType.ARCHON) {
            if (rc.readSharedArray(map_memory_index_0) == 0) {
                for (int memory_dex = 0; memory_dex < total_chunk_count; memory_dex += 4) {
                    if (memory_dex + 4 <= total_chunk_count) {
                        rc.writeSharedArray(map_memory_index_0 + memory_dex / 4, 0b0100_0100_0100_0100);
                    } else {
                        switch (total_chunk_count % 4) {
                            case 1:
                                rc.writeSharedArray(map_memory_index_0 + (memory_dex / 4), 0b0000_0000_0000_0100);
                                break;
                            case 2:
                                rc.writeSharedArray(map_memory_index_0 + (memory_dex / 4), 0b0000_0000_0100_0100);
                                break;
                            case 3:
                                rc.writeSharedArray(map_memory_index_0 + (memory_dex / 4), 0b0000_0100_0100_0100);
                                break;
                        }
                    }
                }
